...
def status_dict(cfg, store: Store, client, positions: List[Dict]) -> Dict[str, Any]:
    ...
    for p in positions:
        ...
        rows.append({
            ...
            "entry": p["entry_price"], "current": cur,
            "stop_loss": p.get("stop_loss"), "take_profit": p.get("take_profit"),
            "pnl_quote": round(pnl, 2),
            "pnl_pct": round((cur / p["entry_price"] - 1) * 100, 2) if p["entry_price"] else 0.0,
        })
    ...
    return {
        "mode": cfg.mode, "market": cfg.market, "env": cfg.env_label,
        "positions": rows, "n_positions": len(rows),
        "unrealized_pnl": round(unreal, 2),
        "equity": equity, "day_pnl": round(day_pnl, 2),
        "kill_switch": bool(getattr(cfg, "risk", None) and store.get_meta(
            f"killswitch:{cfg.mode}:{__import__('datetime').datetime.now(__import__('datetime').timezone.utc).strftime('%Y-%m-%d')}") == "1"),
    }


def show_status(cfg, store: Store, client, positions: List[Dict]) -> None:
    d = status_dict(cfg, store, client, positions)
    _console.print(f"[bold]glmbot[/] | {cfg.env_label} | "
                   f"{d['n_positions']}/{cfg.risk.max_open_positions} positions")
    if d["equity"] is not None:
        _console.print(f"Equity [bold]{fmt_money(d['equity'])} {cfg.quote_asset}[/] | "
    ...
    for col, just in (("Symbol", "left"), ("Qty", "right"), ("Entry", "right"),
                      ("Current", "right"), ("SL", "right"), ("TP", "right"),
                      ("PnL%", "right"), ("PnL", "right")):
        table.add_column(col, justify=just)
    for r in d["positions"]:
        table.add_row(
            r["symbol"], f"{r['qty']:.6f}",
            f"{r['entry']:,.6g}", f"{r['current']:,.6g}",
            f"{r['stop_loss']:,.6g}" if r["stop_loss"] else "-",
            f"{r['take_profit']:,.6g}" if r["take_profit"] else "-",
            colored_pct(r["pnl_pct"]), f"{r['pnl_quote']:+.2f}",
        )
...
def show_positions(positions: List[Dict]) -> None:
    ...
    for col in ("symbol", "strategy", "qty", "entry_price", "stop_loss", "take_profit", "opened_ts"):
        table.add_column(col, justify="right" if col not in ("symbol", "strategy", "opened_ts") else "left")
    for p in positions:
        table.add_row(
            p["symbol"], p["strategy"], f"{p['qty']:.6f}", f"{p['entry_price']:,.6g}",
...
def show_trades(trades: List[Dict]) -> None:
    ...
    for t in trades:
        ...
        table.add_row(
            (t["ts"] or "")[:19], t["mode"], t["symbol"],
            f"[{side_color}]{t['side']}[/{side_color}]",
            f"{t['qty']:.6f}", f"{t['price']:,.6g}", fmt_money(t["quote_amt"]),
...
def trades_summary(trades: List[Dict]) -> Dict[str, Any]:
    ...
    return {"n_trades": len(trades), "n_buys": len(buys),
            "n_sells": len(sells), "total_fees": round(fees, 4)}
...
def show_equity(history: List[Dict]) -> None:
    ...
    _console.print(f"Equity [bold]{fmt_money(last)}[/] ({colored_pct(chg)} since {str(history[0]['ts'])[:10]}) | "
    ...
    for e in history[-20:]:
        table.add_row((e["ts"] or "")[:19], fmt_money(e["cash"]),
                      fmt_money(e["positions_value"]), fmt_money(e["total"]))
    _console.print(table)
...
def show_backtest(results: Dict[str, BTResult], starting: float) -> None:
    ...
    for col, just in (("Symbol", "left"), ("Trades", "right"), ("Win%", "right"),
                      ("PnL%", "right"), ("MaxDD%", "right"), ("PF", "right"),
                      ("Sharpe", "right"), ("Fees", "right"), ("Final", "right")):
        table.add_column(col, justify=just)
    total_final = 0.0
    for sym, r in sorted(results.items()):
        pf = "inf" if r.profit_factor == float("inf") else f"{r.profit_factor:.2f}"
        table.add_row(
            sym, str(r.n_trades), f"{r.win_rate:.0f}%",
            colored_pct(r.pnl_pct), f"{r.max_drawdown_pct:.2f}",
            pf, f"{r.sharpe:.2f}", f"{r.total_fees:.2f}", fmt_money(r.final_equity),
        )
...
def export_backtest_csv(results: Dict[str, BTResult], path: str) -> str:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["symbol", "n_trades", "win_rate", "pnl_pct", "max_dd_pct",
                    "profit_factor", "sharpe", "sortino", "expectancy",
                    "fees", "final_equity"])
        for sym, r in sorted(results.items()):
            w.writerow([sym, r.n_trades, round(r.win_rate, 2), round(r.pnl_pct, 2),
                        round(r.max_drawdown_pct, 2),
                        ("" if r.profit_factor == float("inf") else round(r.profit_factor, 2)),
                        round(r.sharpe, 2), round(r.sortino, 2),
                        round(r.expectancy, 2), round(r.total_fees, 2),
