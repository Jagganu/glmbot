...
class Broker(ABC):
    ...
    def sell_market(
        self, symbol: str, qty: float, price_hint: float, entry_price: float | None = None
    ) -> dict:
...
class PaperBroker(Broker):
    ...
    def __init__(
        self, client: BinanceClient, starting_cash: float, leverage: int = 1, symbol_filter=None
    ):
        self.client = client
        self.cash = float(starting_cash)
        self.leverage = max(1, int(leverage))
        self.fee_rate = (
            FUT_TAKER_FEE if getattr(client, "market", "spot") == "futures" else TAKER_FEE
        )
...
class LiveBroker(Broker):
    ...
    def buy_market(self, symbol: str, quote_amount: float, price_hint: float) -> dict:
        ...
        res = self.client.place_order(
            symbol=symbol, side="BUY", order_type="MARKET", quantity=fmt_dec(qty)
        )
        ...
        return {
            ...
            "quote_amt": executed_qty * fill_price,
            "margin_used": 0.0,
        }
    ...
    def sell_market(
        ...
        if qty_d <= 0:
            raise ValueError(
                f"{symbol}: qty {qty} rounds to 0 (step={f.step_size}) - dust; skipping"
            )
        res = self.client.place_order(
            symbol=symbol, side="SELL", order_type="MARKET", quantity=fmt_dec(qty_d)
        )
        price_avg, fee = self._avg_price_fee(res)
